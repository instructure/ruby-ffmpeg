...
module FFMPEG
  describe Movie do
    describe '#initialize' do
      ...
      let(:movie) do
        allow(Open3).to receive(:capture3).and_return([stdout, '', 0])
        Movie.new(__FILE__)
      end
        ...
        context 'that redirects' do
          ...
          context 'to a relative uri respecting redirect limits' do
            ...
            it 'raise FFMPEG::HTTPTooManyRequests' do
              expect do
                Movie.new('http://127.0.0.1:8000/deep_path/awesome%20movie.mov')
              end.to raise_error(FFMPEG::HTTPTooManyRequests)
            end
          end
          ...
          context 'to a relative uri with too many redirects' do
            it 'should know the file size' do
              expect do
                Movie.new('http://www.toomany-redirects-example.com/moved_movie.mov')
              end.to raise_error(FFMPEG::HTTPTooManyRequests)
            end
          end
        end
      ...
      describe 'special cases' do
        ...
        context 'given a file with bad JSON' do
          let(:stdout_fixture_file) { 'file_with_bad_json.txt' }
        ...
        context 'given an impossible DAR' do
          let(:stdout_fixture_file) { 'file_with_weird_dar.txt' }
        ...
        context 'given an impossible SAR' do
          let(:stdout_fixture_file) { 'file_with_weird_sar.txt' }
        ...
        context 'given a file with ISO-8859-1 characters in output' do
          let(:stdout_fixture_file) { 'file_with_iso-8859-1.txt' }
        ...
        context 'given a file with 5.1 audio' do
          let(:stdout_fixture_file) { 'file_with_surround_sound.txt' }
        ...
        context 'given a file with no audio' do
          let(:stdout_fixture_file) { 'file_with_no_audio.txt' }
        ...
        context 'given a file with non supported audio' do
          let(:stdout_fixture_file) { 'file_with_non_supported_audio_stdout.txt' }
          let(:stderr_fixture_file) { 'file_with_non_supported_audio_stderr.txt' }

          let(:movie) do
            stderr = read_fixture_file("outputs/#{stderr_fixture_file}")
            allow(Open3).to receive(:capture3).and_return([stdout, stderr, 1])
            Movie.new(__FILE__)
          end
        ...
        context 'given a file with non supported audio and video' do
          let(:stdout_fixture_file) { 'file_with_non_supported_audio_and_video_stdout.txt' }
          let(:stderr_fixture_file) { 'file_with_non_supported_audio_and_video_stderr.txt' }
      ...
      context 'given an awesome movie file' do
        ...
        it 'should parse the bitrate' do
          # Different versions of ffprobe might return different bitrates
          expect([481_836, 481_846]).to include(movie.bitrate)
        end
        ...
        it 'should know the video bitrate' do
          expect(movie.video_bitrate).to eq(371_185)
        end
        ...
        it 'should know the sample rate' do
          expect(movie.audio_sample_rate).to eq(44_100)
        end
        ...
        it 'should know the audio bitrate' do
          expect(movie.audio_bitrate).to eq(75_832)
        end
        ...
        it 'should calculate the aspect ratio' do
          # substringed to be 1.9 compatible
          expect(movie.calculated_aspect_ratio.to_s[0..14]).to eq('1.3333333333333')
        end
    ...
    describe '#transcode' do
      ...
      it 'should run the transcoder' do
        ...
        expect(Transcoder).to receive(:new)
          .with(movie, "#{tmp_path}/awesome.flv", options, **kwargs)
          .and_return(transcoder_double)
        expect(transcoder_double).to receive(:run)

        movie.transcode("#{tmp_path}/awesome.flv", options, **kwargs)
      end
    end
    ...
    describe '#screenshot' do
      ...
      it 'should run the transcoder with screenshot option' do
        transcoder_double = double(Transcoder)
        expect(Transcoder).to receive(:new)
          .with(movie, "#{tmp_path}/awesome.jpg", options.merge(screenshot: true), **kwargs)
          .and_return(transcoder_double)
        expect(transcoder_double).to receive(:run)

        movie.screenshot("#{tmp_path}/awesome.jpg", options, **kwargs)
      end
    end
  end
end
