...
module FFMPEG
  describe EncodingOptions do
    ...
    it 'should convert audio sample rate' do
      expect(EncodingOptions.new(audio_sample_rate: 44_100).to_a).to eq(%w[-ar 44100])
    end
    ...
    it 'should specify screenshot parameters when using video quality -v:q' do
      expect(EncodingOptions.new(screenshot: true, vframes: 123,
                                 quality: 3).to_a).to eq(%w[-f image2 -vframes 123
                                                            -q:v 3])
    end
    ...
    it 'should put the parameters in order of codecs, presets, others' do
      ...
      expect(converted).to eq(%w[-i watermark.png -filter_complex scale=,overlay=x=main_w-overlay_w-10:y=10 -vcodec
                                 libx264 -vpre normal -r 25])
    end
    ...
    it 'should specify watermark position at left bottom corner' do
      ...
      converted = EncodingOptions.new(opts).to_a
      expect(converted.find do |str|
               str =~ /overlay/
             end).to include 'overlay=x=main_w-overlay_w-10:y=main_h-overlay_h-10'
    end
